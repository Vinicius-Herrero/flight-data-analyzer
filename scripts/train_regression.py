...
print(f"   Median delay:  {y.median():.1f} min")
print(f"   Std deviation: {y.std():.1f} min")
...
print(f"   Mean delay in sample: {y_sample.mean():.1f} min")
...
metrics_ridge = evaluate_regressor("Ridge Regression", y_test, preds_ridge)

preds_hgb = pipeline_hgb.predict(X_test)
metrics_hgb = evaluate_regressor("HistGradientBoostingRegressor", y_test, preds_hgb)
